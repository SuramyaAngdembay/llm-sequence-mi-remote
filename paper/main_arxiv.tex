...
selected features are almost entirely profile-bound in location
($99.8$--$100\%$ of activation mass on simulated psychometric and
organizational tokens), consistent with user-profile novelty; on
\rfour{}, four of five selected features instead concentrate on
behavioral session events; and subsampling shows that raw positive count
alone does not explain the difference. The feature edits pass intervention
screens against active controls, but they are not repairs: on \rfour{}'s
held-out users the positive patch contrast holds for the best of the searched
edits, and averaged over donors its sign reverses. What reproduces is the pattern,
...
novelty offers a cheap shortcut to sequence-likelihood detectors
(classical detectors on the same features do not take it); when that
shortcut is
...
should find behavioral evidence, and it does. Adapters trained on serializations that differ only in the profile block
show the effect is causal in the audited Qwen3-8B run: with the profile
present, ranking of unseen users sits at chance, and removing profile
content from both adaptation and scoring restores it to about $0.90$ (a
descriptive user-level bootstrap interval excluding zero); the same removal
has a much smaller, non-significant effect in the audited Qwen2.5-3B run, a
model-dependent difference the design does not attribute to capacity alone.
On raw enterprise authentication logs with no injected profile (LANL), a
seen-versus-unseen gap appears but cannot yet be attributed: its evaluation
pools differ in user composition as well as training exposure. The score-level
...
decisively---token-level attribution asks where the selected
features actually fire in the serialized input.

The audit's findings are a dissociation. On CERT \rsix{}, whose evaluation
population contains four malicious users, the selected features are
...
tokens such as durations. Both feature sets pass our intervention screens
against active controls---a searched best-candidate donor contrast and a
necessity ablation. \rfour{}'s donor contrast replicates in direction on
held-out users (feature-level independence; one context individually
significant), while \rsix{}'s held-out replication concentrates on its
dominant user. The donor contrast describes the best of the searched edits,
not a typical edit: rescoring the \rfour{} held-out receivers reproduces it
exactly, while averaging over donors gives negative estimates in every context
(Section~\ref{sec:r42}). Each feature family is stable across SAE
initializations with no counterpart in the other benchmark's dictionary. What
differs is where the selected features are active. Because the session model is adapted on benign users only,
...
general. These conclusions extend beyond the two synthetic benchmarks: the
behavioral pole reproduces on a real-user dataset with genuine psychometrics
(Section~\ref{sec:twos}), and classical detectors given the same profile
fields do not take the shortcut, locating it in the sequence-likelihood
pathway (Section~\ref{sec:discussion}); and on raw enterprise authentication
logs with no injected profile the shortcut recurs in the identity-bearing host
token (Section~\ref{sec:lanl}).
...
\item We test the account's generality on real data and across detector
classes: on TWOS (real users, real Big-Five psychometrics) the audit
finds behavioral evidence exactly where the account predicts it, while
classical one-class detectors given the same CERT profile fields do not
take the profile shortcut, locating it in the sequence-likelihood pathway.
We then test the account causally with train-matched serialization
counterfactuals (Section~\ref{sec:trainfactorial}): four adapters identical
except for the profile block. In the audited $8$B run, ranking of unseen
users with the profile present sits at chance and removing profile content
from adaptation and scoring restores it to $0.90$ (descriptive paired
bootstrap interval excluding zero); the same removal is small and
non-significant in the audited $3$B run, a model-dependent difference not
isolated as capacity, and the psychometric line is the carrier. On raw
enterprise authentication logs with no injected profile
(Section~\ref{sec:lanl}) a seen-versus-unseen gap appears, but its pools
differ in user composition as well as training exposure, so it does not yet
test the account. The resulting two-factor
account: population structure governs when an identity shortcut is
available; objective and representation govern which detectors take it; and
profile content in the serialization is causally responsible for the
binding in the audited run.
...
modern LLMs \citep{du2017deeplog,guo2021logbert}, including benign-trained
event-level language models over enterprise authentication logs whose
strongest variants condition on user identity by construction---per-user
tiered models and per-user score normalization treated as features, not
audited as risks \citep{tuor2018recurrent}. These approaches inherit the
...
Decomposing what fine-tuning changes is the concern of \emph{model diffing}:
sparse crosscoders fit one dictionary across a base model and its fine-tuned
version \citep{lindsey2024crosscoders}, stage-wise diffing adapts a
dictionary to the fine-tuning data or the fine-tuned model
\citep{bricken2024stagewise}, and recent work analyzes LoRA-induced
\emph{delta activations} with sparse autoencoders \citep{prasanth2026lora}.
Our delta-SAE is a single-layer, subtractive form of this idea: it
decomposes the difference $h^{\mathrm{ada}}_t - h^{\mathrm{base}}_t$
directly, which is cheaper than a crosscoder, is blind to cross-layer
structure, and describes the adapter's effect rather than features shared
with the base model. Our contribution is not the delta-SAE construction
itself but its combination with intervention-based validation under matched
controls, user-disjoint security evaluation, and token attribution into an
end-to-end audit. That validity program addresses several problems a recent
survey lists as open for sparse dictionary methods \citep{sharkey2025open}:
reconstruction error (the $\alpha{=}0$ control of Appendix~\ref{app:pkg4}
and the limits of its additive cancellation), dependence of the dictionary on
its training data (benign-only retraining, Section~\ref{sec:cross}), seed
dependence (Section~\ref{sec:seeds}), and the gap between where a latent is
active and what its edits cause (Sections~\ref{sec:attribution}
and~\ref{sec:r42}).
...
prescribes. Benchmark-level critiques have shown that reported gains of
sequence models on standard log-anomaly datasets are partly evaluation
artifacts \citep{landauer2024critical}; our seen-versus-unseen collapse is
the identity-leakage analogue of that critique for insider-threat
benchmarks. Closest to our finding, \citet{kantchelian2024facade} explicitly identify
the pathology in which a detector ``achieves separation by featurizing
actions through the token of their principal'' and engineer around it with
identity-obfuscated contrastive training on proprietary data. We are
complementary: rather than preventing the shortcut by construction, we open
a benign-trained detector and characterize the shortcut
mechanistically---sparse features, causal interventions, attribution
masses, and train-matched counterfactuals
(Section~\ref{sec:trainfactorial})---on public benchmarks where the result
can be reproduced, and we test whether the shortcut arises at all on raw public
authentication logs with no injected profile (Section~\ref{sec:lanl}). On the interpretability side, SAE feature identity is itself an
...
$|g_f|$; proxy-matched variants additionally match activation magnitude and
standardized decoder norm, which narrows but does not equalize the realized
edit size).
...
We keep five kinds of evidence apart: \emph{input sensitivity} (the score
changes when the input changes), \emph{activation location} (where a
feature's activation mass falls), \emph{represented information} (what a
direction encodes), \emph{causal influence on prediction} (editing the
direction changes the model's predictions), and \emph{detection benefit} (an
edit or mask improves discrimination). Evidence for one does not establish
the others. $\tau > 0$ and $\nu > 0$ against active controls are evidence
that edits of $S$ change the score's anomaly evidence more than edits of $C$,
for a joint edit of the whole set under the tested SAE replacement procedure
and with the estimand-specific caveats of Appendix~\ref{app:methods}; in
particular, $\tau$ keeps each receiver's best candidate from a donor and
strength search. The attribution profile $\{m_f\}_{f \in S}$ measures where
$S$ is active. Location constrains what $S$ might encode but does not
establish it. Together, the two support statements of the form ``the
selected features are active on profile (or behavioral) tokens, and editing
them changes the score''. Neither identifies a circuit, and neither shows a
detection benefit.
...
the resulting change in adapted NLL, keeping each receiver's best candidate
(the most negative change over up to $16$ donors and four strengths). Let
...
top/control and benign/anomalous donor values. A positive value means that,
among the searched candidates, the best benign-donor edit of $S$ lowers the
score by more, relative to the best anomalous-donor edit, than the same
comparison for $C$. It describes the best of the searched edits. It is not a
typical donor's effect, an independently chosen intervention policy, or a
detection improvement (Section~\ref{sec:r42}).
...
examples, and more than ablating $C$ does. Throughout, patching is evidence
about a \emph{joint} edit of the whole set under the SAE replacement
procedure: every coordinate of the set is edited at the union of the set's
active positions, including coordinates that were zero there, after the SAE
reconstruction has been substituted at every token. It is not evidence of
literal sufficiency, or of any single feature's role. Ablation establishes
dependence rather than strict necessity, since redundant pathways may exist.
...
primary analysis); open markers denote intervals that cross zero.}
...
controls, the headline contrasts were re-run against \emph{proxy-matched}
controls (features matched on activation frequency, mean active magnitude,
and standardized decoder norm; Appendix~\ref{app:methods}). The proxies
narrow the measured edit-size gap but do not close it (selected edits remain
$1.3$--$1.8\times$ larger), so this is partial size calibration, not an
equal-perturbation test. All four contrasts remain positive, with the
...
$[0.030, 0.055]$) but is negative on the dominant-user fold. The
best-candidate patch contrast therefore transfers to the dominant held-out
user while ablation dependence is user-specific: feature sets selected from
disjoint user subsets both pass the best-candidate screen, but which features
a given user's anomaly evidence depends on varies by user.
...
A later rescoring of the same $638$ held-out receiver days
(Appendix~\ref{app:pkg4}) reproduces these values exactly when each
receiver's best candidate is kept (department $+0.000758$) and shows what
they measure. At full strength ($\alpha{=}1$), the same donor contrast
averaged over donors instead of minimized is negative in every context. Only
the department interval excludes zero ($-0.0028$ on the mean of user means,
$[-0.0045, -0.0012]$, $18$ of $29$ users negative). Averaging over donors
and all four strengths is also negative in every context, again with only the
department interval excluding zero (an exploratory check). The
positive held-out value therefore describes the best of the searched edits.
It does not show that a typical edit toward a benign colleague lowers the
score, and no intervention policy chosen in advance has been shown to do so.
...
\subsection{Where the selected features are active: profile versus behavior}
\label{sec:attribution}

The estimands above are evidence that edits of the selected sparse directions
change the score under the tested procedure, with the best-candidate caveat
of Section~\ref{sec:r42}; they do not say what the directions represent.
Attribution measures where the features are active, which constrains but
does not establish what they represent. We therefore attribute each headline
...
The \rsix{} features are \emph{profile-bound} in location, in a pattern
consistent with user-profile novelty: the benign-only adapter learned the
benign population's profile distribution, and unseen malicious users' profile
fields produce large adapter deltas. This location pattern offers one
explanation of the \rsix{} phenomenology, which no intervention on
represented content has tested: the best-candidate patch effect concentrates
in the dominant user (patching profile tokens toward a context-matched donor
could make the profile look familiar), necessity does not transfer across
users (each user's profile is constant), and feature identity flips when the
dominant user leaves the selection set.
...
Where the features are active is a different question from which
predictions their edits change. The package-4 rescoring of the \rfour{}
held-out receivers (Appendix~\ref{app:pkg4}) splits each edit's loss change by
the class of the predicted token. Because a token is predicted only from
earlier tokens and the profile lines precede every session line, edits at
session positions can change behavior-token loss but not profile-token loss.
With benign donors at full strength, averaged over donors, the selected edits
raise behavior-token loss by more than the control edits ($+0.016$ to
$+0.018$ nats per token across the four contexts, every interval excluding
zero; at strengths $0.25$ and $0.5$ the difference is slightly negative). The
differential effect on profile-token loss is uncertain (every interval
includes zero), and the preregistered direct comparison of the two classes is
inconclusive (every interval includes zero). We therefore claim neither that
profile loss is unaffected nor that the two effects differ. The contexts share
users and are not independent replications. The edit is joint: every selected
coordinate is written at the union of the set's active positions, so no
profile effect can be assigned to the one \texttt{DAY}-bound feature. The
selected edits are also several times larger than the control edits, and
every receiver is a malicious day. These results therefore show that the joint
edit changes behavior-token predictions under this procedure. They do not
show that the features represent malicious behavior, that the effect exceeds
what an equally large perturbation at the same positions would cause, or that
it is specific to anomalous days.
...
induces sparse token-level structure whose edits pass the audited
intervention screens on both benchmarks, with held-out replication that is
directional on \rfour{} and dominant-user-concentrated on \rsix{}; the
structure is benchmark-specific not merely in coordinates but in where it is
active: identity metadata on \rsix{}, behavior on \rfour{}. This subsection stress-tests
...
dominant-user fold. Where the selected features are active is therefore
...
\input{tables/dictionary_robustness.tex}
...
sessions on $12$ of them ($50\%$ prevalence) plus five staged insider
(``traitor'') sessions, and---critically---each
...
windows ($18{,}048$ windows; $138$ positive, of which $101$ are masquerade
and $37$ traitor windows; $12$-user benign train pool),
...
The audit finds behavioral evidence on this benchmark: top-5 attribution
...
(mean within-user ROC $0.783$ across the $16$ attacked users---$12$
masqueraded plus $4$ traitors with logged activity---identity held fixed
by construction); and the intervention estimands reproduce the
...
\subsection{Real auth-log replication with no injected profile (LANL)}
\label{sec:lanl}
...
CERT's profile block is injected by the benchmark, so a natural objection is
that the shortcut is an artifact of injection. The two-factor account makes a
sharper prediction: wherever the serialization carries a per-user-constant,
identity-informative token, a sequence-likelihood detector will shortcut on
\emph{that} token, whatever field it is. We test this on the LANL
``Comprehensive, Multi-Source Cyber-Security Events'' authentication log
\citep{kent2015comprehensive}: $58$ days of raw Windows authentication events
from a real enterprise ($12{,}425$ users, $1.05$ billion events), with $749$
red-team compromise events (credential-based lateral movement) over $104$
users as ground truth and no profile of any kind. Each event carries only identity tokens (source and
destination user, source and destination host) and behavioral tokens
(authentication type, logon type, orientation, outcome, hour). We serialized
user-days into $32$-event windows (a window is positive iff it contains a
red-team event; the longest window is $1{,}503$ tokens against a
$2{,}048$-token context, so no window was truncated), keeping all
red-team users and a $1/10$ sample of benign users ($1.62$M windows, $432$
positive), adapted Qwen2.5-3B benign-only under the recipe of
Section~\ref{sec:trainfactorial}, and scored held-out windows by adapted NLL.
Users are hashed into five folds; adapters train on benign windows from four
folds and are evaluated on \emph{seen}-fold and \emph{unseen}-fold users
separately. Four serializations differ only in the identity tokens: the full
line, user identity anonymized, host identity anonymized, and user identity
deranged across users. Positive windows are rare (prevalence $1.0\%$ among seen-fold windows and
$0.65\%$ among unseen-fold windows), so we report both ROC-AUC and average
precision.

\begin{center}
\begin{tabular}{lcccc}
\toprule
Identity tokens in serialization & Seen AUC & Seen AP & Unseen AUC & Unseen AP \\
\midrule
...
Three observations. First, the seen-versus-unseen collapse reproduces on real
data with no injected profile: every serialization ranks seen users near
$0.95$ and unseen users far lower, at chance except when hosts are
anonymized. Second, the \emph{user} token contributes little: anonymizing or
deranging it moves unseen-user AUC by at most $0.02$ (a fixed permutation
relabels a user rather than removing the label), so the shortcut is not
carried mainly by the user identifier. Third, the \emph{host} token carries
it, with the trade-off seen for CERT's profile block: anonymizing hosts costs
seen-user ranking ($0.949\!\to\!0.877$) and improves unseen-user ranking
($0.497\!\to\!0.627$), although unseen-user average precision stays at the
base rate ($0.013$ at prevalence $0.65\%$) in every condition, so the gain is
in ranking, not in alert precision. Hosts are user-associated rather than
per-user-constant: a user's most frequent source host accounts for a median
$54\%$ of that user's events, and only $7\%$ of users issue $80\%$ or more of
their events from one host. The host field is therefore a partial identity
proxy that may also encode resource access and attack-path structure, and
its analogy to CERT's constant \texttt{PSY} line is qualitative. A
seen-versus-unseen gap remains within the host-anonymized condition ($0.877$
versus $0.627$), which may reflect behavioral heterogeneity across users,
residual identity cues, or both. Two caveats: this is a single $3$B adapter
per condition, and the ground truth is red-team credential misuse rather than
CERT's insider scenarios, so these numbers replicate the \emph{mechanism}, not
a detection benchmark. Notably the host shortcut is severe even in this $3$B
adapter, whereas on \rsix{} the profile-removal effect appears only in the
audited $8$B run (Section~\ref{sec:trainfactorial}), consistent with the
availability factor being a property of the data.

\subsection{Train-matched serialization counterfactuals: profile content causes the collapse in the audited 8B run}
\label{sec:trainfactorial}

Every causal result above intervenes at \emph{inference} time on an adapter
trained with the profile present, so profile capture could be a correlate of
training rather than a consequence of the profile. To close this gap we trained
four adapters from scratch under an identical recipe on \rsix{} serializations
that differ \emph{only} in the profile block: (i) \textsc{full}; (ii)
\textsc{no-psy}, dropping the \texttt{PSY} line; (iii) \textsc{no-profile},
dropping \texttt{DAY} and \texttt{PSY}; and (iv) \textsc{shuffle}, a seeded
derangement that gives every user another user's profile---token presence,
format, per-user constancy, and surface statistics preserved; only the
original correspondence between a user and the recorded attributes is broken
(the reassigned profile still acts as a stable pseudonym). We
ran the factorial at both scales used in this paper (Qwen2.5-3B and the
headline Qwen3-8B; benign-only QLoRA, $300$k examples, one adapter seed per
scale) and scored each adapter under the fold-aligned protocol of
Section~\ref{sec:detprotocol} (one held-out malicious user against all $406$ benign users available in
this pool per fold---the protocol's cap of $800$ exceeds the pool, so the four
folds share one comparison cohort---with day-level ROC and user-level ROC after
max-aggregation over days; \rsix{}'s four malicious users give four folds).
Each condition changes the serialization used for both adaptation and
scoring, so the contrast measures the effect of profile content in the
pipeline, not a training-only pathway. Uncertainty is the user-level cluster bootstrap ($10{,}000$ draws over the
four malicious users), applied \emph{paired} across arms; with four clusters
these intervals are descriptive, conditional on the trained adapters and the
fixed comparison cohort, and they do not characterize variation across
training runs.

\begin{center}
\begin{tabular}{lcc}
\toprule
Training serialization & 3B unseen-user ROC & 8B unseen-user ROC \\
\midrule
\textsc{full}       & $0.926$ [$0.84$, $0.98$] & $0.532$ [$0.25$, $0.82$] \\
\textsc{no-psy}     & $0.903$ [$0.87$, $0.93$] & $\mathbf{0.906}$ [$0.88$, $0.93$] \\
\textsc{no-profile} & $0.956$ [$0.88$, $0.99$] & $\mathbf{0.903}$ [$0.84$, $0.96$] \\
\textsc{shuffle}    & $0.656$ [$0.50$, $0.81$] & $0.485$ [$0.29$, $0.68$] \\
\midrule
removal (\textsc{no-profile}$-$\textsc{full}) & $+0.03$ [$-0.07$, $+0.14$] & $\mathbf{+0.37}$ [$+0.07$, $+0.69$] \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{In the audited 8B run the collapse is caused by profile content and is reversible.}
The $8$B \textsc{full} adapter collapses on unseen users (user-level ROC
$0.532$; day-level $0.399$), qualitatively reproducing the headline collapse of
Section~\ref{sec:detresults} on a capped-corpus adapter. Removing the profile
restores user-level ROC to $0.903$, a paired removal effect of $+0.371$ whose
descriptive bootstrap interval excludes zero (one-sided bootstrap tail
fraction $0.005$). The four per-user differences are $-0.05$, $+0.82$,
$+0.42$, and $+0.29$: the effect is carried by three of the four users, and
the interval characterizes evaluation uncertainty for these two adapters
rather than replacing replication across adapter seeds. Dropping the
\texttt{PSY} line alone recovers as much ($0.906$): the psychometric line is
the carrier, and the \texttt{DAY} line adds little. Because each condition
changes the serialization used for both adaptation and scoring, the result
attributes the collapse to profile content in the pipeline rather than to a
training-only pathway; within that scope it is causal, not a correlate.

\paragraph{In the audited 3B run the shortcut is captured but removal has little effect.}
The $3$B \textsc{full} adapter already generalizes ($0.926$; its day-level ROC
of $0.741$ is in line with the better of the paper's two repaired-serializer
$3$B seeds, $0.738$, Section~\ref{sec:cross}), and removal changes little
($+0.03$, interval spanning zero). Yet the delta-SAE audit of the same adapter
is profile-dominated (four of five top features at $\approx\!100\%$ profile
mass). Capture without a measurable removal effect in the $3$B run, capture
with a large one in the $8$B run: the difference is consistent with the
capacity account of Section~\ref{sec:discussion}, but the design does not
isolate capacity. Qwen2.5-3B and Qwen3-8B differ in model generation and
post-training as well as size, each arm has one adapter seed (the paper's own
$3$B \rsix{} adapters vary from $0.414$ to $0.738$ day ROC across seeds), and
a non-significant $3$B effect is not evidence of no effect.

\paragraph{The 8B collapse is concentrated, and psychometric familiarity tracks it.}
The $8$B collapse is not uniform across the four malicious users: one remains
detected (user-level ROC $0.919$), two sit at chance ($0.527$), and one is
\emph{inverted}---ranked $344$th of $407$ test users, among the least
anomalous. With the profile removed, all four are ranked in the top fifth of
the cohort at both scales (ranks $10$--$75$ of $407$ at $8$B, $4$--$64$ at
$3$B; user-level ROC $0.82$--$0.99$); the one user the $8$B \textsc{full}
adapter still detected moves slightly down ($0.919\!\to\!0.867$) and the three
collapsed users move up. A direct check of the mechanism against the $3{,}590$
benign training users: the inverted user has the least novel \texttt{PSY}
profile of the four ($12$ benign users within $L_1\!\le\!10$ in OCEAN space)
and the surviving user one of the most novel ($3$); the two most
psychometrically familiar profiles belong to the inverted user and to one of
the two chance-level users. The most novel profile ($0$ neighbours) belongs
to the other chance-level user, which is however the best detected at day
level ($0.653$), so novelty orders the users only approximately. The
organizational \texttt{DAY} tuple is not predictive (the surviving user shares
its role, department, and team with $29$ benign users). Ablation and novelty
point to the same carrier: the $8$B adapter memorizes psychometric
familiarity strongly enough that a malicious user whose OCEAN profile
resembles many benign trainees is scored as familiar and ranked below the
unfamiliar unseen benigns. With four users this is consistent with, not proof
of, the mechanism.

\paragraph{Shuffling.} Deranging profiles never helps. At $8$B the
\textsc{shuffle} adapter sits with \textsc{full} at chance ($0.485$); at $3$B
it falls well below the honest-profile adapter ($0.656$ versus $0.926$).
The derangement preserves per-user constancy, and because it is a bijection
each profile text is carried by exactly one user, so an unseen user's
borrowed profile is as unseen during adaptation as its own would have been:
exposure is unchanged, and what is lost is only the correspondence between
the profile's content and the user's recorded attributes and behavior. The
result shows that profile content matters to unseen-user ranking in both
audited runs; it does not identify whether broken attribute--behavior
associations or particular profile similarities explain the loss. A fixed
profile that belongs to someone else is worse than none, even for a model
for which the honest profile is harmless.

\paragraph{Capture survives shuffling; utility does not.}
Running the delta-SAE audit on the \textsc{full} and \textsc{shuffle} adapters
separately dissociates what the model represents from what helps detection.
Profile capture does not disappear when profiles are deranged: at $3$B the
top-five features are profile-bound under both adapters (four of five under
\textsc{full}, five of five under \textsc{shuffle}), and at $8$B both adapters
place two hard-profile features ($\approx\!1.00$ profile mass) among their top
five. The derangement keeps each user's borrowed profile constant across that
user's days, so the per-user constancy that makes profile tokens compressible
for a next-token model is preserved; only the original correspondence between profile content and the user's
behavior and labels is broken; the reassigned profile remains a stable
pseudonym for the user. Capture tracks per-user constancy, which shuffling
leaves intact, while ranking of unseen users tracks whether profile content
is informative about those users, which shuffling degrades: under
\textsc{shuffle} the audit reports the same profile capture alongside
chance-level unseen-user detection. This is a controlled, ground-truth
instance of the paper's methodological point that attribution mass alone
overstates a shortcut, and only a behavioral or causal probe distinguishes an
informative binding from a merely salient one.
...
evaluation showed strong ROC on both benchmarks; edits of the selected
features passed at least one audited intervention screen on both (a
best-candidate patch contrast or a necessity ablation);
...
metadata-rich telemetry, intervention tests and token-level attribution are
separate obligations. The first asks whether editing a direction changes the
score, the second where the direction is active; together they constrain
what a detector responds to, and neither establishes what a direction
represents. In this sense the audit's mechanistic layer is demonstrably
...
alone: only conjoined with intervention tests and score-level evaluation
does it indicate what a detector responds to. One might object that we audit a
...
matters. The refined account also makes a testable prediction we leave to
future work: sequence autoencoders, which share the sequence-prediction
objective but not the pretrained representation, should fall between the
tabular detectors and the LM in profile reliance; auditing their latent
spaces with this framework would locate the shortcut's origin more finely.
Auditing the class before it is deployed is the point.
...
the shortcut. The two factors have been manipulated to differing degrees. The
availability factor is now tested directly: the train-matched factorial of
Section~\ref{sec:trainfactorial}---matched adapters differing only in whether
the profile block is present, dropped, or shuffled---shows that removing profile content from the serialization restores
unseen-user ranking in the audited $8$B run and that shuffling it (tokens
present, attribute correspondence broken) leaves cross-user ranking at or
below the profile-present level, establishing that profile content in the
serialization is causally responsible for the collapse in that run rather
than being a correlate. The second factor is less cleanly
isolated: the classical detectors differ from the language model in objective,
representation, architecture, and capacity simultaneously, and profile
distinctiveness has not been varied independently of malicious-user count.
The remaining decisive tests are therefore those that isolate the
representation--objective factor (e.g.\ loss-downweighting of profile tokens,
or distinctiveness varied at fixed count), replication of the removal effect across adapter seeds, a design that
separates training exposure from scoring representation, and a
size-controlled comparison within one model family. The account's two corners
...
Section~\ref{sec:twos}). A third setting, raw enterprise authentication logs
with no injected profile (LANL; Section~\ref{sec:lanl}), was meant to test
whether the shortcut is tied to profile injection, but its seen and unseen pools
differ in user composition as well as training exposure, so it does not yet
answer that question. The cross-architecture
probe adds the second
factor: classical one-class detectors on the same \rsix{} features do not
take the shortcut, so ``cheap'' is relative to the learner. The account in
its refined form is two-factor---\emph{population structure determines
when the shortcut is available; the objective and representation determine
which detectors take it}. Twelve profile columns among $242$ are not
especially cheap for a density model, but per-user-constant profile tokens
are maximally cheap for a sequence-likelihood objective. This also
explains a number reported earlier: under user-disjoint evaluation the
session LM \emph{underperforms} the classical baselines on \rsix{}
($0.545$ against $0.63$--$0.77$), exactly as expected if adaptation spent
capacity on a shortcut that does not generalize while the baselines
learned behavior. For benchmark design the
implication is accordingly conditional but sharp: exposing static per-user
metadata, especially simulated psychometrics with no operational
counterpart, creates a capture risk for sequence-likelihood detectors that
cannot be predicted from population size and that score-level evaluation
will not notice---it must be audited for. For
...
finds, tests, and locates candidate mechanisms---not the features.
...
circuit-level account \citep{chan2022causal}. The causal-patching estimand
keeps each receiver's best candidate from a donor and strength search. On the
\rfour{} held-out receivers the mean over donors has the opposite sign
(Appendix~\ref{app:pkg4}), so neither a typical-edit benefit nor an
intervention policy is established. Every patch edits the whole feature set at
the union of its active positions after substituting the SAE reconstruction
at every token. The reconstruction-only term cancels additively between arms
but can interact with the edits inside the network, and a residual-preserving
edit ($h + \mathrm{dec}(z') - \mathrm{dec}(z)$) has not been applied to the
reported estimands. The proxy-matched controls narrow the edit-size
difference without removing it. Receivers are malicious days only, so no
effect is shown to be specific to anomalies rather than to ordinary
predictive disruption. Attribution is based on activation mass over line classes; it identifies
...
high-prevalence, real-psychometrics corner, and the LANL replication
(Section~\ref{sec:lanl}), which supplies an organization-scale real population
with no injected profile; what remains untested is the profile-capture corner
on real data---a small, profile-distinctive malicious minority within an
organization-scale population---which no public real dataset currently
provides.
...
are on simulated data and on small or single-run real datasets: they establish methodology, not operational readiness.
...
\emph{both} arms whenever the receiver has any active feature of the arm,
the arms share a reconstruction-only term. On the \rfour{} held-out receivers,
an $\alpha{=}0$ run confirms that this term is numerically identical across
arms (Appendix~\ref{app:pkg4}), so it cancels additively in the contrast.
It does not cancel inside the network. Writing $h$ for the hidden state, $R$
for the reconstruction replacement and $E_S, E_C$ for the edits, the contrast
is $L(h{+}R{+}E_S) - L(h{+}R{+}E_C)$, not $L(h{+}E_S) - L(h{+}E_C)$, and
the nonlinear downstream computation can change the response to either edit.
Note also that step (3) writes every coordinate of $F$ at every position in
$T_F$, including coordinates that were zero there, so the arms are joint
edits at the union of their active positions. The recorded per-receiver outcome for each (feature set, donor
...
arms. As a partial answer to dose-matching, we additionally constructed
\emph{proxy-matched} active controls---features matched to the top-5 on
activation frequency, mean active magnitude, and standardized decoder-column
norm while remaining gap-neutral---which narrows the edit-size gap to
$1.33\times$ (\rsix{}) and $1.79\times$ (\rfour{}), and reran the headline
contexts. These are proxies. The match does not use the realized
donor-minus-receiver coefficient changes, the decoder norm omits the
inverse-standardization factor $\sigma$ applied to the actual patch, joint
directions and their cross terms are not matched, and neither are the edited
positions or their token classes. The comparisons below are therefore not
equal-perturbation tests.
...
retains at least one individually significant estimand against proxy-matched
controls.
...
\rsix{}, $+0.0006$ on \rfour{}). That estimand still selects each donor's
best strength. Without any search (the mean over donors at fixed
$\alpha{=}1$), the \rfour{} held-out contrast is negative in every context
(Appendix~\ref{app:pkg4}).
...
\subsection{Package 4: token-class rescoring of the \rfour{} held-out receivers}
\label{app:pkg4}
The $638$ held-out receiver days ($30$ confirmation users) were rescored
with the published interventions (all $530{,}792$ candidate keys match the
published run) while recording each edit's loss change by the class of the
predicted token (\texttt{DAY} and \texttt{PSY}: profile; \texttt{SESCOUNT}
and \texttt{SES}: behavior). A reconstruction-only run ($\alpha{=}0$) covered
the same receivers. The analysis rules were declared before either run
produced rows. A receiver's value is the mean over all candidate donors at
the given strength. The aggregate is the mean of receiver-user means with a
user-clustered percentile bootstrap. Table~\ref{tab:pkg4} gives the primary
comparison.

\begin{table}[h]
\centering
\small
\caption{Package 4, benign donors, $\alpha{=}1$: selected minus control
loss change (nats per token), mean of user means with $95\%$ user-clustered
intervals. The four contexts share users and are not independent
replications. The direct comparison (last column) is the preregistered
primary quantity.}
\label{tab:pkg4}
\begin{tabular}{lccc}
\toprule
Context & Behavior tokens & Profile tokens & Profile $-$ behavior \\
\midrule
department & $+0.0172$ $[0.0107, 0.0238]$ & $+0.0069$ $[-0.0064, 0.0290]$ & $-0.0104$ $[-0.0267, 0.0144]$ \\
dept.$\times$role & $+0.0158$ $[0.0088, 0.0231]$ & $+0.0086$ $[-0.0058, 0.0333]$ & $-0.0073$ $[-0.0251, 0.0202]$ \\
role & $+0.0156$ $[0.0086, 0.0227]$ & $+0.0082$ $[-0.0057, 0.0317]$ & $-0.0073$ $[-0.0249, 0.0188]$ \\
team & $+0.0179$ $[0.0110, 0.0249]$ & $+0.0065$ $[-0.0056, 0.0266]$ & $-0.0114$ $[-0.0270, 0.0115]$ \\
\bottomrule
\end{tabular}
\end{table}

Every receiver has active features of both sets, so both arms replace the
same days with their reconstruction. The reconstruction-only effect agrees
across arms to within $2\times10^{-7}$ nats per receiver, and the
selected-minus-control differences are the same raw and net of
reconstruction. That cancellation is additive only (see the patching procedure above). The
separate $\alpha{=}0$ run was scored in differently composed batches, and its
unpatched scores differ from the main run's by about $0.007$ nats on profile
tokens and $0.0005$ on behavior tokens (user means), so absolute
$\alpha{=}1$ minus $\alpha{=}0$ effects are not read as isolated feature
edits. No view's $90\%$ interval lies inside the declared equivalence margin
($\pm0.0002$ nats), so no effect is called negligible. At strengths $0.25$ and
$0.5$ the behavior difference is slightly negative ($-0.0005$ to $-0.0011$);
at $0.75$ it is $+0.005$ to $+0.006$. Anomalous donors give the same pattern.

The published best-candidate difference-in-differences is reproduced to six
decimals when the best candidate is chosen against one base per receiver
(department $0.000758$, dept.$\times$role $0.000446$, role $0.000340$, team
$0.000398$). The mean over donors at $\alpha{=}1$ is negative in every
context. Receiver-weighted values are $-0.0045$, $-0.0017$, $-0.0016$ and
$-0.0020$. On user means, the department value is $-0.0028$
$[-0.0045, -0.0012]$ and the other intervals include zero. Equalizing the
benign and anomalous candidate counts leaves the best-candidate value almost
unchanged, so the sign difference reflects best-candidate selection, not
unequal pool sizes.

The preregistration declared $5{,}000$ bootstrap draws, and the analysis code
used $10{,}000$. With $5{,}000$ draws, interval bounds move by at most
$0.0063$ nats, on the widest intervals. $17$ of $2{,}160$ intervals change
whether they exclude zero. Each has a bound within $0.0006$ of zero, and all
are absolute single-arm effects, secondary views, or behavior differences at
$\alpha{=}0.5$. No primary quantity changes.
