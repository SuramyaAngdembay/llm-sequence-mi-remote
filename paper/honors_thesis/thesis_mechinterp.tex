...
\label{eq:tau}
\end{equation}
Two things cancel. The unedited score $\bar{\ell}$ appears in all four changes. And within one
feature set, the benign-donor and malicious-donor edits reconstruct the same tokens of the same
day, so the reconstruction's own effect is the same in both and cancels in $a_{\mathcal{F}}$ and
in $a_{\mathcal{C}}$. What remains compares the edits alone.
...
\subsection{Reading}
Two things are established. The features that most distinguish malicious days activate almost
only on session tokens. And on users held out from feature selection, the choice between a benign
and a malicious donor matters more when the selected features are edited than when the control
features are, by a small margin. Three things are not established: what information the features
represent; which token predictions their edit changes, since the edit is measured here on the
full score only and an experiment that splits the same edits by token class is in progress; and
whether the profile shortcut of \chref{ch:lm} arises mainly from averaging surprisal over every
token, since the score decomposition shows that the shortcut is removable at scoring time but not
where it arises inside the model.
...
donor-specific effect. The planned analysis averages over all candidates instead.
