...
finite. This plays the role that the smoothing parameter $\alpha$ plays for the CTMC
(Section~\ref{sec:ctmc-mle}).
\end{itemize}
...
\end{equation}
This is exactly the CTMC surprisal of \chref{ch:ctmc}, except that the condition is the entire
preceding text rather than a single previous state. Section~\ref{sec:why-log} explains why
$-\log p$ is the right measure of surprise.
...
text: its negative log-likelihood under the model. This is the additivity of
Section~\ref{sec:why-log} with one refinement. Tokens are not independent, so each token's
