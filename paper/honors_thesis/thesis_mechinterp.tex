...
entire preceding text rather than a single previous state, and Section~\ref{sec:why-log}
explains why $-\log p$ is the right measure of surprise. A token the model expects is cheap;
...
The model also assigns a probability to the whole text of a day. By the chain rule of
probability, the probability of a sequence is the product of each token's probability given
...
text: its negative log-likelihood under the model. This is requirement (R2) of
Section~\ref{sec:why-log} applied to a sequence. For example, if a three-token continuation
has token probabilities $\tfrac12$, $\tfrac12$ and $\tfrac14$, the tokens cost $1$, $1$ and $2$
bits, a total of $4$ bits; and the whole continuation has probability
...
accumulates more total surprisal simply by being longer. The anomaly score of Section~\ref{sec:lm-score}
therefore divides the total by the number of scored tokens, giving the surprisal per token.
