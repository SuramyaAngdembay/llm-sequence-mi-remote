...
should certify behavioral evidence, and it does. Train-matched adapters
differing only in the profile block localize the effect to training time: at
the headline $8$B scale, unseen-user detection with the profile present sits
at chance and removing the profile restores it (paired cluster bootstrap,
$p=0.005$), while the same removal is inert at $3$B, so profile availability
during adaptation causes the collapse and its harm emerges with capacity. On
raw enterprise authentication logs with no injected profile (LANL), the
unseen-user collapse recurs and the shortcut relocates to the identity-bearing
host field. The score-level
...
certifies behavioral evidence exactly where the account predicts it, while
...
We then ground the account causally at training time with a train-matched
factorial (Section~\ref{sec:trainfactorial}): four adapters identical except
for the profile block. At the headline $8$B scale, unseen-user detection with
the profile present sits at chance and removing the profile restores it to
$0.90$ (paired cluster bootstrap, $p=0.005$); the same removal is inert at
$3$B, so the shortcut's harm is causal and emerges with capacity, and it is
carried by the psychometric line. On raw enterprise authentication logs with
no injected profile (Section~\ref{sec:lanl}) the unseen-user collapse recurs
and the shortcut relocates to the identity-bearing host field. The resulting
two-factor account: population structure governs when an identity shortcut is
available; objective and representation govern which detectors take it; and
profile availability at training time is causally responsible for the
binding.
...
benchmarks. Closest to our finding, \citet{xu2024facade} explicitly identify
...
red-team lateral-movement events over $104$ users as ground truth and no
profile of any kind. Each event carries only identity tokens (source and
...
red-team event, so truncation can never drop a positive), keeping all
...
deranged across users. Because positive windows are rare ($432$ of $1.62$M),
average precision is dominated by the base rate and we report ROC-AUC.

\begin{center}
\begin{tabular}{lcc}
\toprule
Identity tokens in serialization & Seen-user AUC & Unseen-user AUC \\
\midrule
full (user $+$ host)     & $0.949$ & $0.497$ \\
user anonymized          & $0.944$ & $0.518$ \\
host anonymized          & $0.877$ & $\mathbf{0.627}$ \\
user deranged (shuffle)  & $0.947$ & $0.488$ \\
\bottomrule
\end{tabular}
\end{center}

Three observations. First, the seen-versus-unseen collapse reproduces on real
data with no injected profile: every serialization scores seen users near
$0.95$ and unseen users near chance. Second, the \emph{user} token is inert:
anonymizing or deranging it changes neither number, so the shortcut is not
carried by the user identifier. Third, the \emph{host} token carries it, with
exactly the trade-off seen for CERT's profile block: anonymizing hosts costs
seen-user detection ($0.949\!\to\!0.877$) and buys unseen-user generalization
($0.497\!\to\!0.627$). This is localization rather than failure: on an
authentication log users log on from their own machines, so the host is the
per-user-constant, identity-informative token, playing the role CERT's
\texttt{PSY} line plays. A residual unseen-user gap survives host removal
($0.63$, not $0.95$), which we read as genuine behavioral heterogeneity across
users rather than shortcut. Two caveats: this is a single $3$B adapter per
condition, and the ground truth is red-team lateral movement rather than
CERT's insider scenarios, so these numbers replicate the \emph{mechanism}, not
a detection benchmark. Notably the unseen-user collapse is already severe at
$3$B here, whereas on \rsix{} it emerges only at $8$B
(Section~\ref{sec:trainfactorial}): raw host identity is a stronger shortcut
than an injected profile at small scale, consistent with the availability
factor being a property of the data.

\subsection{Train-matched counterfactuals: the shortcut's harm is causal and emerges with capacity}
...
format, and surface statistics preserved, profile--identity link destroyed. We
...
Section~\ref{sec:detprotocol} (one held-out malicious user against $800$
benign test users per fold, user-level ROC after max-aggregation over days;
\rsix{}'s four malicious users give four folds). Uncertainty is the paper's
primary instrument, the user-level cluster bootstrap ($10{,}000$ draws),
applied \emph{paired} across arms for the removal contrast.
...
\midrule
removal (\textsc{no-profile}$-$\textsc{full}) & $+0.03$ [$-0.07$, $+0.14$], $p{=}0.32$ & $\mathbf{+0.37}$ [$+0.07$, $+0.69$], $p{=}0.005$ \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{At the headline scale the collapse is causal and reversible.}
...
bootstrap interval excludes zero ($p=0.005$); the cluster bootstrap
establishes the effect from a single adapter, so it does not rest on seed
replication. Dropping the \texttt{PSY} line alone recovers just as much
($0.906$): the psychometric line is the carrier, and the \texttt{DAY} line adds
little. Profile availability during adaptation is therefore causally
responsible for the unseen-user collapse rather than a correlate of it.

\paragraph{At 3B the shortcut is captured but not yet harmful.}
...
$3$B seeds, $0.738$, Section~\ref{sec:cross}), and removal changes nothing
($+0.03$, not significant). Yet the delta-SAE audit of the same adapter is
profile-dominated (four of five top features at $\approx\!100\%$ profile
mass). Capture without harm at $3$B, capture with harm at $8$B: the shortcut's
\emph{damage} to generalization is capacity-emergent, the training-time face of
the capacity argument in Section~\ref{sec:discussion}. (The paper's $3$B
\rsix{} adapters vary widely across seeds, $0.414$ versus $0.738$ day ROC, so
the $3$B statement is a single-seed one.)

\paragraph{The 8B harm is concentrated, and psychometric familiarity predicts it.}
...
anomalous. Removing the profile rescues every user at both scales
($0.82$--$0.99$). A direct check of the mechanism against the $3{,}590$ benign
training users: the inverted user has the least novel \texttt{PSY} profile of
the four ($12$ benign users within $L_1\!\le\!10$ in OCEAN space) and the user
best detected at day level has the most novel ($0$); the two most
psychometrically familiar users are exactly the two that collapse, while the
...
converge on the same carrier: the $8$B adapter memorizes psychometric
familiarity strongly enough that a malicious user whose OCEAN profile resembles
many benign trainees is scored as familiar and ranked below the unfamiliar
unseen benigns. With four users this is consistent with, not proof of, the
mechanism.
...
it falls well below the honest-profile adapter ($0.656$ versus $0.926$): a
profile that lies misleads even a model for which an honest profile is
harmless.
...
for a next-token model is preserved; only the correlation with true identity
is destroyed. Capture tracks compressibility, which shuffling leaves intact,
while detection tracks identity-informativeness, which shuffling removes: under
...
the profile block is present, dropped, or shuffled---shows that removing the
profile improves unseen-user generalization and that shuffling it (tokens
present, identity link destroyed) collapses cross-user ranking, establishing
that profile \emph{availability at training time} is causally responsible for
the binding rather than being a correlate. The second factor is less cleanly
...
or distinctiveness varied at fixed count), and confirmation of the training-time
result at the $8$B scale of our headline detector. The account's two corners
...
signal relocates to the per-user-constant host field. The cross-architecture
