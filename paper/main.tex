...
out dictionary non-identifiability as the source of transfer failure. A
...
seen-versus-unseen-user effect. The audit thus surfaces failure modes
invisible to leaderboards---anomaly evidence resting on identity and novelty
rather than behavior---and motivates a population-structure hypothesis: that what such mechanisms encode tracks the
...
on \rfour{} with a residual behavioral component ($0.668$ day ROC against
unseen benigns). This also explains the ranking--precision dissociation: user
novelty separates users (ROC) but cannot rank a user's malicious days above
their benign days (PR). We therefore treat detection as background and ask
