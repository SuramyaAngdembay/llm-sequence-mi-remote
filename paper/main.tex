...
tokens---simulated psychometric traits and organizational identifiers---showing the selected causal features are overwhelmingly profile-bound,
consistent with \emph{user-profile novelty}; on \rfour{} (sixty
malicious users), four of five top features are behavioral, concentrated on
session-event tokens such as durations. Cross-seed decoder alignment shows each mechanism is stable across SAE
initializations for the same cached adapter representations
($|\cos|~0.88$--$0.96$ across seeds, versus a $0.08$--$0.11$ empirical null
across benchmarks), ruling out dictionary non-identifiability as the source
of transfer failure. A
score-level sensitivity analysis completes the picture: against never-trained
benign users, day-level ROC collapses to $0.55$ (\rsix{}) and $0.67$
(\rfour{}), so the apparent detection strength is largely a
seen-versus-unseen-user effect. A malicious-population subsampling study
on \rfour{} then rejects the natural explanation that positive-population
size alone drives the dissociation: even four-user positive subsamples select
predominantly behavioral features, so \rsix{}'s profile capture reflects
benchmark-specific structure rather than a generic small-population effect.
The audit thus surfaces failure modes invisible to leaderboards---anomaly
evidence resting on identity and novelty rather than behavior---shows they
cannot be predicted from population size alone, and makes ``which benchmark
properties drive metadata capture'' a precisely posed open question.
\end{abstract}
...
encode. Because the model is trained on benign users only, malicious-population
structure cannot alter training; it can only shape which existing directions
the \emph{audit selects}. We therefore tested the natural audit-selection
hypothesis---small positive populations favor profile-bound selection---by
subsampling \rfour{}'s malicious users, and found it insufficient: even
four-user subsamples select predominantly behavioral features. \rsix{}'s
...
(user-level: $0.970\!\to\!0.546$ and $0.969\!\to\!0.565$). Since
malicious users are unseen by construction, the unseen-versus-unseen
comparison is the fair test of behavior detection, and it shows the
fold-aligned ROC advantage is largely a \emph{seen-versus-unseen-user}
effect. The fold-aligned numbers below follow the local baselines' protocol
...
rather than by the ROC value itself. A user-novelty account is also
\emph{consistent with} the ranking--precision dissociation---novelty
separates users but not a user's malicious days from their benign
days---though confirming this requires within-user day-ranking metrics we
leave to the appendix materials. We therefore treat detection as background and ask
what internal structure the benign-only adaptation induced.
...
cross-benchmark comparison (18 versus 26) sits at the empirical null, but that
partly reflects a depth difference; at matched layer 18 the two benchmarks'
top features are modestly \emph{above} their null baselines (mean $|\cos|$
$0.26$ and $0.43$ versus baselines $0.19$ and $0.35$), as expected since both
SAEs encode the same base model's layer-18 geometry. The decisive contrast is
scale: matched-layer cross-benchmark alignment ($0.26$--$0.43$) remains far
below within-benchmark cross-seed alignment ($0.88$--$0.96$), so the
distinct headline mechanisms are not a relabeling of one shared circuit even
...
on both benchmarks, with held-out replication that is directional on
\rfour{} and dominant-user-concentrated on \rsix{}; the structure
...
malicious-population structure. We tested the population-size hypothesis directly by malicious-population
subsampling on \rfour{}: holding the model, benchmark, SAE, and benign
population fixed, we resampled positive populations of $K=4,10,20,30,60$
users (ten repetitions each) and re-ranked features for each. The strong form
of the hypothesis is \emph{rejected}: profile-mass share among top-5 features
...
populations tilt selection toward profile features only mildly. \rsix{}'s
...
question the audit makes precise. A score-time ablation completes the picture. We re-serialized the
...
rescored with the frozen models on the unseen-versus-unseen population
(positives against never-trained benign users). On \rsix{}, masking changes
essentially nothing (day ROC $0.547$ original, $0.575$ no-psy, $0.471$
no-profile): with every user unseen, profile novelty separates nothing, and
no behavioral signal is revealed underneath. On \rfour{}, masking
psychometrics \emph{improves} discrimination substantially (day ROC
$0.670\!\to\!0.797$; day PR $0.069\!\to\!0.112$; user ROC
$0.666\!\to\!0.775$): the simulated psychometric fields were not merely
...
Finally, excluding profile-dominated features from selection reveals that
...
size---fails our direct test: four-user subsamples of \rfour{} still select
...
adapter happened to compress into its deltas). For benchmark design the
...
\emph{both} arms, reconstruction-substitution error is common to top and
control interventions and cancels in the top-minus-control contrast; the arms
differ only in the feature edit at their active positions. The recorded
per-receiver outcome for each (feature set, donor type) is the best (most
negative) NLL change over the candidate-donor pool and $\alpha$ grid; best-donor
selection is applied identically to top and control arms.
