...
differing only in the profile block localize the effect to training time:
shuffling profiles across users---tokens present, identity destroyed---
collapses cross-user ranking, indicating that profile availability during
adaptation is causally responsible for the shortcut rather than a correlate
of it. The score-level
...
pathway (Section~\ref{sec:discussion}).
...
for the profile block, where shuffling profiles across users---presence
preserved, identity link destroyed---collapses cross-user ranking to chance
while removal yields the best unseen-user generalization. The resulting
two-factor account: population structure governs when an identity shortcut
is available; objective and representation govern which detectors take it,
and profile availability at training time is causally responsible for the
binding.
...
can be reproduced. On the interpretability side, SAE feature identity is itself an
...
\subsection{Train-matched counterfactuals: profile availability is causal at training time}
\label{sec:trainfactorial}

Every causal result above intervenes at \emph{inference} time on an adapter
that was trained with the profile present; a fair objection is that profile
capture might be a correlate of training rather than a consequence of the
profile itself. To close this gap we trained four adapters from scratch under
an identical recipe (Qwen2.5-3B, benign-only QLoRA, $300$k training examples,
shared seed) on \rsix{} serializations that differ \emph{only} in the profile
block: (i) the full serialization; (ii) \textsc{no-psy}, dropping the
\texttt{PSY} line; (iii) \textsc{no-profile}, dropping both \texttt{DAY} and
\texttt{PSY}; and (iv) \textsc{shuffle}, a seeded derangement that gives every
user another user's profile block---token presence, line format, and surface
statistics are preserved exactly, but the profile--identity link is destroyed.
We score each adapter by adapted NLL under a lightweight protocol (positives
against held-out benign users for a user-disjoint ROC; per-user day ranking
for a within-user ROC; $70$ positive user-days; one seed). Absolute values
under this protocol are not comparable to the fold-aligned benchmark of
Section~\ref{sec:detprotocol}; the object of interest is the contrast across
arms, which shares protocol, data volume, and seed.

\begin{center}
\begin{tabular}{lcc}
\toprule
Training serialization & User-disjoint ROC & Within-user ROC \\
\midrule
full                & $0.736$ & $0.739$ \\
\textsc{no-psy}     & $0.683$ & $0.702$ \\
\textsc{no-profile} & $\mathbf{0.774}$ & $0.570$ \\
\textsc{shuffle}    & $\mathbf{0.452}$ & $0.666$ \\
\bottomrule
\end{tabular}
\end{center}

Three observations. First, the \textsc{shuffle} arm isolates
\emph{informativeness} from \emph{presence}: with profile tokens intact but
decoupled from identity, cross-user ranking collapses to chance or below
($0.452$, $-0.284$ versus full) while within-user ranking survives
($0.666$)---the model learns to bind score structure to profile content
during training, and when profiles are uninformative that binding actively
misleads ranking across users. No inference-time intervention can produce
this contrast. Second, removing the profile entirely yields the best
cross-user generalization ($0.774$), and removing \texttt{PSY} alone does not
($0.683$): the \texttt{DAY} line suffices to carry the shortcut, so the
effect is not specific to the simulated psychometric inventory. Third, the
profile block is not purely parasitic: \textsc{no-profile} \emph{degrades}
within-user ranking ($0.739\!\to\!0.570$) even though the profile is constant
within a user, indicating that profile context also conditions the behavioral
representation. Serializing identity-linked profile therefore buys
within-user sensitivity at the cost of cross-user generalization---a
trade-off, not a free defect---which is the training-time face of the
two-factor account developed in Section~\ref{sec:discussion}. These are
pilot-scale results (one seed, $3$B, $70$ positives) and we scope them as
directional; they causally ground, but do not numerically transfer to, the
$8$B findings above.
...
separately dissociates \emph{what the model represents} from \emph{what helps
detection}. Profile capture does not disappear when profiles are deranged: at
the audited layer the top-five features are profile-bound for both adapters---four
of five under \textsc{full} (profile mass $\{1.00, 0.85, 1.00, 0.03, 0.99\}$)
and, if anything, more so under \textsc{shuffle} (five of five, mass
$\{1.00, 0.92, 1.00, 1.00, 0.99\}$). The reason is that the derangement assigns
each user a \emph{different} profile but keeps it constant across that user's own
days, so the per-user constancy that makes profile tokens maximally compressible
for a next-token model is preserved; what the shuffle destroys is only the
correlation between profile and true identity. Capture therefore tracks
compressibility, which shuffling leaves intact, while the cross-user ranking
collapse above ($0.452$) tracks identity-informativeness, which shuffling
removes. The two come apart cleanly: under \textsc{shuffle} the audit reports
$\sim\!100\%$ profile capture alongside \emph{below-chance} cross-user detection.
This is a controlled, ground-truth instance of the paper's central
methodological point---attribution mass alone overstates a shortcut, and only a
behavioral or causal probe distinguishes an informative binding from a merely
salient one. (This readout is $3$B, single-seed; the $8$B confirmation is
in progress.)
...
Section~\ref{sec:twos}). The cross-architecture probe adds the second
