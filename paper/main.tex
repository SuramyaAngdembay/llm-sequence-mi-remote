...
different anomaly evidence in the audited adaptation runs (one per
benchmark). On \rsix{}, the causally implicated sparse
...
(Section~\ref{sec:seeds}), but adapter-seed and architecture-level
robustness are not: our evidence covers one model family at one scale with
one adapter training run per benchmark, and all dissociation claims should
be read as scoped to these runs. The planned smaller-scale serializer-repaired
rerun will double as an adapter-seed replication. Relatedly, the
