...
Because the derangement preserves per-user constancy, what is lost is not a
stable per-user token but the correspondence between the profile's content
and its owner; and because the permutation runs over the whole population, an
unseen user can carry a training user's profile, so profile familiarity no
longer tracks whether a user was seen. A fixed profile that belongs to someone
else is worse than none, even for a model for which the honest profile is
harmless.
