...
anomaly language models---sparse autoencoders over adapter deltas, causal
patching and necessity ablation against matched active controls,
cluster-level uncertainty, held-out-user evaluation, seed-stability controls,
and token-level attribution---and apply it to a Qwen3-8B model adapted with
QLoRA on benign users only, across two CERT insider-threat benchmarks.
Despite similar score-level behavior, the benchmarks induce qualitatively
different anomaly evidence. On \rsix{}, the causally implicated sparse
features are almost entirely profile-bound, placing $99.8$--$100\%$ of their
activation mass on simulated psychometric and organizational tokens,
consistent with user-profile novelty; on \rfour{}, four of five selected
features concentrate on behavioral session events such as durations. Each
feature family is stable across SAE initializations, while literal feature
transfer between benchmarks fails. Critically, when malicious users are
compared only against benign users never seen during adaptation, day-level
ROC-AUC falls from $0.95$ to $0.55$ (\rsix{}) and from $0.96$ to $0.67$
(\rfour{}): apparent detection strength is largely a seen-versus-unseen-user
effect. Malicious-population subsampling on \rfour{} rejects positive-population
size as the explanation for profile capture, and score-time profile masking
\emph{improves} \rfour{}'s unseen-user discrimination, showing profile
metadata can actively obscure behavioral signal. High anomaly-ranking
performance can thus conceal qualitatively different---and sometimes
identity-proxy---internal evidence; mechanistic auditing makes the difference
visible.
\end{abstract}
...
alignment is indistinguishable from that null baseline, showing that SAE
initialization instability is unlikely to explain the observed transfer
failure.
...
independence; we flag this wherever held-out results are claimed. The headline \rsix{} configuration uses layer $18$,
...
masking and swap experiments of Section~\ref{sec:cross} provide score-level
...
(including positive rows), so held-out analyses are dictionary-conditional;
benign-only dictionary retraining is the corresponding robustness check. SAE-seed robustness is established
