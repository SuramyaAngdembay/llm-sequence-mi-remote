...
positive for every user, is concentrated in one heavily active user---yet a
leave-one-user-out test recovers a significant causal effect on that user from
features selected without any of their data.
...
sparsity and survives a held-out confirmation: with features selected on half
of the malicious users, causal effects on the untouched half remain positive
in all four organizational contexts, at roughly half the exploratory
magnitude. The evidence supports a specific conclusion:
...
configuration frozen. All four causal contexts remain positive on held-out
users, at roughly half the exploratory magnitude
($0.00034$--$0.00076$), which is the expected signature of selection
inflation in the exploratory estimates. The department context is significant
under user-level cluster resampling ($0.00076$, cluster CI
$[0.00035, 0.00121]$; $21$ of $29$ users positive), department$\times$role
under receiver-level resampling ($[0.00006, 0.00084]$), and department and
team under mean-of-user-means weighting. Held-out necessity replicates for
department$\times$role and role under user-mean weighting, while team is a
confirmed null. The \rfour{} mechanism is thus confirmed in direction on
untouched users---though smaller than its \rsix{} counterpart, at a
different depth, with a sparser dictionary---and rests on a far broader user
population.
