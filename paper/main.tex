...
causally validated sparse mechanisms: \rsix{} effects survive same-user
exclusion and a leave-one-user-out confirmation; \rfour{}'s natively
rediscovered mechanism remains positive on thirty held-out malicious users
with features selected from the other thirty. It then reveals what those
...
evidence resting on identity metadata rather than behavior---together with a
boundary condition: what a benign-trained session model encodes tracks the
structure of the benchmark's malicious population.
\end{abstract}
...
tokens such as durations. Both mechanisms are causally real---each survives
held-out-user confirmation---and each is a stable, seed-robust subspace of its
own model with no counterpart in the other benchmark's dictionary. What
differs is what they encode, and the natural explanation is the structure of
the malicious population: with four positive users, user identity is the
cheapest separator; with sixty, the identity shortcut dilutes and behavioral
structure carries the evidence.
...
\rfour{}---a failure mode and its boundary condition, both invisible to
score-level evaluation.
\item We show the two mechanisms are stable subspaces of their own models
(cross-seed decoder alignment $0.88$--$0.96$ against $\sim$$0.6$ chance) with
chance-level correspondence across benchmarks, ruling out SAE
...
exclusion, and discovery/confirmation splits repair them.
\end{enumerate}
...
The result is unambiguous and opposite between benchmarks. On \rsix{}, the
five top causal features place $99.8$--$100\%$ of their activation mass on
profile tokens---four on the \texttt{PSY} line (enrichment $\approx$$13\times$
...
($|\cos|$ $0.88$--$0.96$ against dictionary-median baselines of
$\sim$$0.58$--$0.65$); across benchmarks, best matches sit at the chance
floor ($0.08$--$0.11$) in both directions for all three seeds per side.
...
malicious-population structure. A score-time ablation that masks profile
fields and rescores both populations with the frozen model is in progress and
will quantify each benchmark's score-level dependence on profile tokens.
