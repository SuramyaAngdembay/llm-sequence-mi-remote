...
what part of the input those directions respond to. On one standard benchmark the evidence the audit selects turns out to be
...
second benchmark it is mostly behavior. When malicious users are compared only against normal users
...
A train-matched retraining experiment then shows that, in the audited
eight-billion-parameter run, profile content in the serialization causes the
collapse: removing the profile block from the text used for adaptation and
scoring restores unseen-user ranking from chance to about $0.90$ (a
descriptive paired resampling interval over the four malicious users
excludes zero), while the same removal has a small, non-significant effect
in the audited three-billion-parameter run; the two runs differ in model
family as well as size and were trained once each, so the design does not
attribute the difference to capacity alone. The personality line carries the
effect. On a corporate login log with no profile of any kind, the collapse
on unseen users recurs and host identity, a user-associated field, carries
the shortcut.
\end{abstract}
...
Figure~\ref{fig:architecture} draws the whole chain as an architecture
before we walk through it step by step.
...
\end{equation}
The double difference protects the comparison: an effect that comes merely
from making an edit of that size, rather than from the specific content of
the selected features, appears in both arms and is removed to the extent
that the control edit matches the treatment edit in strength, which the
paper enforces by matching the size of the control edit to the treatment
edit.
...
ablation $\nu = 0.065$ with interval $[0.026, 0.083]$). The evidence the audit selects, the five features that most raise the
score, is almost entirely a description of \emph{who the user is}.
...
the internal analysis. Replacing a malicious day's profile lines with a random benign user's
profile changes the score more than replacing its entire behavioral content
on both benchmarks (mean score change $+0.155$ against $+0.072$ on r6.2, a
ratio of $2.2$; $+0.180$ against $+0.102$ on r4.2, a ratio of $1.8$). The
profile swap dominates on both; the behavioral swap is a larger share of the
total on r4.2, in line with the more behavioral audit there, but the two
benchmarks are not reversed. And the masking result of
...
dismissed users, only days before the dismissal enter training), trained
twice with
...
\section{Real data with no profile at all}
\label{sec:lanl}

TWOS still contains a profile line, albeit a real one, so a determined
skeptic can maintain that the shortcut needs \emph{some} injected profile
to exist. The two-factor account makes a sharper prediction than that:
wherever the serialized text carries a field that is constant for each
user and reveals who the user is, a likelihood-based detector will
shortcut on \emph{that} field, whatever it is called. We tested this on
the ``Comprehensive, Multi-Source Cyber-Security Events'' dataset released
by Los Alamos National Laboratory in 2015: fifty-eight days of raw Windows
...
window is $1{,}503$ tokens against a $2{,}048$-token context, so no window
was truncated. We kept every red-team user and a one-in-ten sample of benign
...
folds ($300{,}729$ windows) and were evaluated separately on seen-fold
...
\begin{tabular}{lcccc}
\toprule
Identity fields in the serialization & Seen AUC & Seen AP & Unseen AUC & Unseen AP \\
\midrule
...
anonymized. Second, the user field contributes little: anonymizing or
...
carried mainly by the user identifier. Third, the host field carries
...
its analogy to CERT's constant personality line is qualitative. A
...
adapter, whereas on CERT r6.2 the profile-removal effect appears only in the
audited $8$B run (Section~\ref{sec:factorial}), consistent with the
...
\section{Is the profile the cause? A train-matched experiment}
\label{sec:factorial}

Every causal result so far intervenes at scoring time on a detector that
was trained with the profile present, so profile capture could in
principle be a correlate of adaptation rather than a consequence of the
profile. The direct test is a controlled experiment that changes the text used for adaptation and scoring. We
trained four adapters from scratch under an identical recipe on r6.2
texts that differ \emph{only} in the profile block: (i) \textsc{full};
(ii) \textsc{no-psy}, with the personality line removed; (iii)
\textsc{no-profile}, with both profile lines removed; and (iv)
\textsc{shuffle}, a seeded derangement that gives every user another
user's profile, so that token presence, format, and surface statistics
and per-user constancy are preserved; only the original correspondence
between a user and the recorded attributes is broken, and the reassigned
profile still acts as a stable pseudonym. We
ran this factorial at both scales used in the paper, Qwen2.5-3B and the
headline Qwen3-8B ($300{,}000$ benign training examples, one adapter seed
per scale).

Each adapter was scored under the strictest protocol available, the
fold-aligned protocol of Section~\ref{sec:protocols}: each of r6.2's four
malicious users is held out in turn and ranked against all $406$ benign users of this pool never used in training
(the protocol's cap of $800$ exceeds the pool), a user's score is the maximum of its daily
scores, and the AUC is averaged over the four folds. Uncertainty is the cluster bootstrap that resamples the four malicious users
ten thousand times; because the same users are resampled for every arm, it
also yields a paired interval for the difference between two arms. With four
malicious users these intervals are descriptive: they are conditional on the
trained adapters and the fixed comparison cohort, and they say nothing about
variation across training runs.

\begin{center}
\begin{tabular}{lcc}
\toprule
Training serialization & 3B unseen-user AUC & 8B unseen-user AUC \\
\midrule
...
\emph{In the audited 8B run the collapse is caused by profile content and is reversible.}
The 8B \textsc{full} adapter collapses on unseen users (user-level AUC
$0.532$; day-level $0.399$), qualitatively reproducing the headline collapse of
Section~\ref{sec:collapse} on a capped-corpus adapter. Removing the profile
restores user-level AUC to $0.903$, a paired removal effect of $+0.371$ whose
...
personality line alone recovers as much ($0.906$): the psychometric line is
the carrier, and the organizational header adds little. Because each condition
...
\emph{In the audited 3B run the shortcut is captured but removal has little effect.}
The 3B \textsc{full} adapter already generalizes ($0.926$; its day-level AUC
of $0.741$ is in line with the better of the paper's two repaired-serializer
3B seeds, $0.738$, Section~\ref{sec:robustness}), and removal changes little
($+0.03$, interval spanning zero). Yet the dictionary audit of the same adapter
is profile-dominated (four of five top features at $\approx\!100\%$ profile
mass). Capture without a measurable removal effect in the 3B run, capture
with a large one in the 8B run: the difference is consistent with the
capacity account of Section~\ref{sec:classical}, but the design does not
...
3B r6.2 adapters vary from $0.414$ to $0.738$ day-level AUC across seeds), and
a non-significant 3B effect is not evidence of no effect.

\emph{The 8B collapse is concentrated, and psychometric familiarity tracks it.}
The 8B collapse is not uniform across the four malicious users: one remains
detected (user-level AUC $0.919$), two sit at chance ($0.527$), and one is
...
the cohort at both scales (ranks $10$--$75$ of $407$ at 8B, $4$--$64$ at
3B; user-level AUC $0.82$--$0.99$); the one user the 8B \textsc{full}
...
profile of the four ($12$ benign users within $L_1\!\le\!10$ in the five-dimensional Big Five space)
...
organizational header is not predictive (the surviving user shares
its role, department, and team with $29$ benign users). Ablation and novelty
point to the same carrier: the 8B adapter memorizes psychometric
familiarity strongly enough that a malicious user whose personality scores
...
\emph{Shuffling.} Deranging profiles never helps. At 8B the
\textsc{shuffle} adapter sits with \textsc{full} at chance ($0.485$); at 3B
...
\emph{Capture survives shuffling; usefulness does not.} Running the
dictionary audit on the \textsc{full} and \textsc{shuffle} adapters
separately dissociates what the model represents from what helps it.
Profile capture does not disappear when profiles are deranged: at 3B the
top five features are profile-bound under both adapters (four of five
under \textsc{full}, five of five under \textsc{shuffle}), and at 8B both
adapters place two features of essentially pure profile mass among their
top five. The derangement keeps each user's borrowed profile constant
across that user's days, so the per-user constancy that makes profile
tokens cheap to predict is preserved; only the original correspondence between profile content and the user's
...
leaves intact, while usefulness tracks whether the profile content is
informative about the unseen users being ranked, which shuffling degrades: under \textsc{shuffle} the audit
reports the same profile capture alongside chance-level ranking of unseen
users. This is a controlled instance of the methodological point that
runs through the paper: attribution alone overstates a shortcut, and only
a behavioral or causal probe distinguishes an informative binding from a
merely salient one.
...
by an out-of-setting prediction that the real-user dataset confirmed, by a train-matched retraining experiment showing that profile content in the serialization causes the unseen-user collapse in the audited 8B run
(Section~\ref{sec:factorial}), and by a login log with no profile at all,
where the shortcut relocates to the host field (Section~\ref{sec:lanl});
the second by the cross-detector comparison. The retraining experiment adds a qualification the account did not
anticipate: the shortcut was captured in both audited runs but measurably
damaged unseen-user ranking only in the larger one, a model-dependent
difference that the single-seed, cross-family design does not attribute to
capacity alone.
...
the real-data studies are small; the retraining experiment uses one adapter seed per scale, its intervals
coming from resampling the four malicious users, which characterizes
evaluation uncertainty for those adapters and not variation across training
runs; and the login-log
study is a single 3B adapter per condition with a red-team rather than
insider ground truth. What survives every variation we tested
