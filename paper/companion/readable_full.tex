...
or claim a detection leaderboard advance. Appendix~\ref{app:related}
...
The checks below separate a stable finding from a result that depends on
how it was measured. The consistent result is the contrast in selected
activation locations. Repair and ablation do not replicate equally well.
Appendix~\ref{app:robustness} gives the supporting numbers.

\emph{Does selection work on new users?} On r4.2, features selected with
thirty malicious users were evaluated on the other thirty. Four of five
features reappear. Held-out repair effects are positive but about half
as large as the exploratory effects; only the department context has a
user-bootstrap interval excluding zero. A rescoring of the same held-out days
reproduces this best-candidate value exactly, but the mean over donors at
full strength is negative in every context (department $-0.0028$, interval
$[-0.0045, -0.0012]$; the others include zero). The held-out result therefore
describes the best searched edits, not a typical one
(Appendix~\ref{app:pkg4}). Held-out ablation is positive
in some contexts, while team is a null. On r6.2, leaving one malicious
user out of selection shows that repair concentrates on the user who
contributes $46$ of the $70$ positive days. Held-out ablation of the
original profile features does not transfer to that dominant user.
The dictionary and configuration were chosen using the full population,
so these checks establish independence of feature selection and testing
conditional on those choices, not a wholly independent rediscovery.

\emph{Is the dictionary an accident of initialization?} Three dictionary
seeds per benchmark recover closely aligned directions: absolute cosine
similarities $0.81$--$0.95$, against dictionary-wide reference medians
$0.72$--$0.82$. Cosine similarity compares the angle between two
normalized directions. Across benchmarks the selected directions align
at or below their reference medians, including when both dictionaries
are compared at layer 18. Fresh dictionary seeds also reproduce positive
r6.2 repair contrasts. This addresses dictionary randomness for fixed
adapter representations; it does not establish reproducibility across
training runs of the language model.

\emph{Does including malicious data in the dictionary create the finding?}
The original dictionaries include positive rows. Training new dictionaries
only on benign rows reproduces the attribution pattern: at least $94$
percent session mass on r4.2 and at least $99$ percent profile mass in
every r6.2 leave-one-user-out selection. The r4.2 ablation contrast
replicates in all contexts, but its repair contrast becomes slightly
negative. On r6.2, held-out repair and ablation depend on which user
is tested. The selected locations are more reproducible than the
individual intervention effects.

\emph{Is four versus sixty malicious users the explanation?} Holding the
r4.2 model and dictionary fixed, we reselected features from populations
of $4,10,20,30,$ and $60$ malicious users, with ten repetitions per size.
Profile share is about $0.25$ at four users and $0.16$--$0.20$ at larger
sizes, far below r6.2. Count alone therefore does not explain selection
in this fixed dictionary. The test does not isolate an effect of count
on dictionary formation or prove a particular alternative explanation.

\emph{Does the pattern survive new adapters and a repaired serializer?}
Four file-routing fields were accidentally omitted from the original
r6.2 serialization; r4.2's release contains one of those fields. After
repairing this defect, we trained new low-rank adapters on the pretrained
Qwen2.5-3B model, using two seeds per benchmark and new benign-only
dictionaries. The selected r4.2 features concentrate on sessions under
both seeds. At the deepest tested r6.2 layer, three or four of the five
features are almost entirely profile-bound, while shallower layers can
be behavioral. The r4.2 ablation effect replicates under both seeds
($+0.00218$ and $+0.00215$); repair does not (about $-0.0004$ each).
r6.2 intervention effects vary by seed and measurement. These are
replications of the attribution pattern and r4.2 ablation dependence,
not of every original causal estimate.

\emph{Is behavioral evidence absent from r6.2?} No. Restricting selection
to session positions and excluding profile-dominated features reveals a
second feature family. It carries held-out repair on the dominant user
($+0.0027$--$+0.0029$), with partial held-out ablation dependence.
Unrestricted selection prioritizes profile features; it does not show
that the detector contains no behavioral causal structure.

\emph{Do input changes agree with the internal audit?} Swapping a
positive day's profile with an unseen benign user's profile changes
mean NLL more than replacing all its session content on \emph{both}
benchmarks: $+0.155$ versus $+0.072$ on r6.2, and $+0.180$ versus
$+0.102$ on r4.2. These probes measure sensitivity to an input change,
not the benefits of familiar profiles, because donors were unseen.
Thus raw score sensitivity is profile-dominated in both models even
though their selected features activate at different locations. The
masking and within-user tests help separate this sensitivity from
...
The additional datasets and interventions suggest that population
structure and the training representation influence the available
shortcuts. They do not establish a two-factor law. Subsampling changes
feature selection in an existing model; TWOS shows a behavioral
counterpart with real questionnaires; LANL shows a host-sensitive
score-level generalization gap; and the classical comparison changes
several modeling choices together. The strongest direct input evidence
is the improvement after removing profiles from adaptation and scoring
in the audited 8B run. Its small comparison in 3B and the persistence of
profile attribution under shuffling show why attribution alone is
insufficient to diagnose harm.
...
The original 8B mechanism estimates concern one adapter per CERT release,
one-layer edits, and the matched session domain. The headline dictionaries
and configuration search use evaluation data; held-out feature selection
is therefore conditional on them. The repaired 3B reruns and benign-only
dictionaries provide additional checks, not independent replication of
every 8B effect. Repair is a best-candidate search, and on r4.2's held-out
users the donor-averaged effect has the opposite sign. Every edit is a joint
edit applied after replacing the delta with its reconstruction at every
token; that replacement cancels additively between arms but can interact with
the edits, and a residual-preserving edit has not been applied to the
reported estimands. Receivers are malicious days only, so no effect is shown
to be specific to anomalies. TWOS is small and staged; LANL uses staged
credential misuse and one held-out fold. No result establishes deployment readiness,
a complete neural circuit, or a general effect of model size. The appendix
records the protocols, quantitative controls, and artifact paths supporting
these boundaries. Code and the reported result artifacts are available in
\href{https://github.com/SuramyaAngdembay/llm-sequence-mi-remote}{the project repository};
raw datasets and model checkpoints are separate inputs.

\paragraph{Implications for people.}
An insider-threat detector is also a workplace-monitoring system.
Profile sensitivity can turn unusual personal attributes into suspicious
scores, and an explanation can give weak evidence unwarranted authority.
Benign-only training does not remove these risks. These experiments support
auditing such systems, not autonomous action against individuals; any
operational use would require appropriate human review and governance.

\clearpage
\appendix
\input{readable_appendix.tex}
