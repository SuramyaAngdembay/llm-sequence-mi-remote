...
mechanism. The full paper tests, through intervention experiments,
whether editing these selected features changes the anomaly score: one
edits the coefficients $z_{t,f}$ of the selected features, propagates the
edited internal state through the rest of the network, and compares the
score change with the change from editing active control features. The
selected edits pass these tests, with two qualifications that limit the
conclusion. The ``repair'' result keeps the best of many searched edits;
averaged over the donor days instead, the corresponding r4.2 effect on
held-out users has the opposite sign. And the selected edits are larger
than the control edits and change all selected features together. The
experiments therefore show that editing the selected set can change the
score. They do not show what the features represent, or that any single
feature matters. Figure~\ref{fig:effects} summarizes those measurements.
...
malicious users as the resampling unit. Positive values favor the
selected set over the active control in the stated contrast; the patch
(``repair'') contrast keeps each day's best searched edit. Open circles
mark intervals that include zero.}
\label{fig:effects}
\end{figure}
...
investigated, not merely asked. The chain presented here, namely
differences of internal states, a sparse dictionary approximation,
selection of score-relevant directions, and the location of those
directions in the input, turns the question into measurable quantities.
In the full paper this chain is tested by intervention experiments and an
extensive robustness program, and it is that combination, within the
limits the full paper states, rather than any single number, that
supports the conclusions summarized here.
