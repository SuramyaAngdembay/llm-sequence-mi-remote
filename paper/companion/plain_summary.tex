...
determine what its score is based on. On one standard benchmark the
score turns out to rest almost entirely on who each user is (their
recorded personality and organizational profile) rather than on what
they did; on a second benchmark the score rests mostly on behavior. When
...
trained on the same information do not take the profile shortcut. Retraining the detector with the profile removed shows that the profile
is the cause and not a bystander: at the large model, removal restores
ranking of unseen users from chance to about $0.90$, while at a smaller
model it makes no difference, so the harm requires capacity. On a
corporate login log with no profile of any kind, the same failure appears
and the shortcut moves to the computer name. The conclusion is that high
...
the observed separation (average within-user AUC about $0.7$).
...
profile text at all. Every version ranks seen users well and unseen users
at chance. Hiding or scrambling the user name changes nothing, so the
shortcut is not the user name. Hiding the computer names costs some
seen-user quality and buys unseen-user quality, which is exactly the
trade-off produced by CERT's profile lines. The reason is simple: people
log on from their own machines, so on a login log the computer name is
the field that is constant for each user and reveals who they are, and it
plays the role that the personality line plays in CERT. A gap remains
after the computer names are hidden ($0.63$ rather than $0.95$), which we
read as genuine differences between people's behavior rather than as a
shortcut. Two cautions apply: each of these four detectors was trained
once, and the intrusions are staged credential theft rather than the
insider scenarios of CERT, so the numbers replicate the mechanism rather
than benchmark a product.
...
user's profile throughout, so the text keeps the same form and the same
statistics but the profile no longer tells the truth about its owner.

For each detector we used the strictest evaluation available. Each of the
four malicious users in r6.2 is held out in turn and ranked against $800$
benign users who were never used in training; a user's score is the
...
paired test of the difference between two detectors.
...
\midrule
removal effect (both removed $-$ full) & $+0.03$ ($p = 0.32$) & $\mathbf{+0.37}$ ($p = 0.005$) \\
\bottomrule
\end{tabular}
\end{center}
...
With the profile removed it ranks them at $0.903$. The difference,
$+0.37$, has a resampling interval that excludes zero ($p = 0.005$): the
profile is not merely noticed by the detector; its presence during
training \emph{causes} the failure on unseen users, and removing it
repairs the detector. Removing the personality line alone repairs it just
...
and removing the profile changes nothing measurable ($+0.03$). Yet when we
open the small model by the method of Section~\ref{sec:plain-inside}, its
selected features are just as profile-bound as the large model's. The
small model has \emph{noticed} the profile without being harmed by it;
the harm appears only when the model is large enough to memorize each
user's profile so thoroughly that familiarity swamps behavior. The harm,
in other words, is a property of capacity, which is one reason the size
of the headline model matters.
...
user is precisely the one whose five personality scores most closely
resemble those of many benign training users; the user whose scores are
most unusual is the one the model still catches. A large model trained on
normal users learns that familiar personality scores mean ``normal,'' and
a malicious user who happens to have ordinary scores is waved through.
Removing the profile catches all four users, at both model sizes.
Scrambling the profiles never helps, as one would expect: a description
that lies is at best useless and at worst misleading. One further
...
the data representation determines \emph{which} detectors take it. The retraining experiment adds
that availability is causal rather than merely correlated with the
failure, and the login-log experiment adds that the shortcut lands on
whichever field is constant per user and reveals identity: the computer
name there, the personality line here.
