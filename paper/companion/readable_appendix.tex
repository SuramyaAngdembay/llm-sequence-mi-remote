...
with $h$ the hidden state, $R$ the replacement and $E_S,E_C$ the edits, the
contrast is $L(h{+}R{+}E_S)-L(h{+}R{+}E_C)$, not $L(h{+}E_S)-L(h{+}E_C)$.
Every coefficient of $F$ is written at every position in $T_F$, including
coefficients that were zero there, so each arm is a joint edit at the union
...
Controls additionally matched on the proxies activation frequency,
magnitude, and standardized decoder norm reduce the selected/control norm
ratio to $1.33$ on r6.2 and $1.79$ on r4.2. The match does not use the
realized donor-minus-receiver coefficient changes, the inverse-standardization
factor $\sigma$ of the actual patch, the cross terms of joint directions, or
the edited positions and their token classes. The rerun results are:
\begin{center}\small
\begin{tabular}{lcc}
\toprule
...
and $0.0006$ (r4.2). That estimand still selects each donor's best strength.
Without any search (the mean over donors at $\alpha=1$), the r4.2 held-out
contrast is negative in every context (Appendix~\ref{app:pkg4}). Dividing each best-row score change by its edit
...
\subsection{Package 4: token-class rescoring of the r4.2 held-out receivers}
\label{app:pkg4}
...
reconstruction. That cancellation is additive only (see the repair definition above). The
...
$[0.00035,0.00121]$, $21/29$ contributing users positive). These are
best-candidate values; rescoring the same days reproduces them exactly, and
the mean over donors at $\alpha=1$ is negative in every context
...
\section{Relation to earlier work and artifact guide}
\label{app:related}

Earlier insider-threat work models aggregated behavior and log sequences
\citep{tuor2017deep,le2020analyzing,du2017deeplog,guo2021logbert}.
Event-level recurrent language models also use user context
\citep{tuor2018recurrent}; more recent systems include LoRA-based DMFI
and LLM-based RedChronos \citep{song2025dmfi,sun2025redchronos}.
Facade uses benign-trained contextual anomaly detection with a different
objective and operational setting \citep{kantchelian2024facade}.
Our results do not establish that these systems share the audited failure.
Context can be informative; the question is whether its use supports
the intended generalization and alerting task.

Sparse dictionaries, activation interventions, and analyses of LoRA delta
geometry have prior precedents
\citep{gao2024scaling,zhang2023best,prasanth2026lora}. Decomposing what
fine-tuning changes is the concern of model diffing: crosscoders fit one
dictionary across a base model and its fine-tuned version
\citep{lindsey2024crosscoders}, and stage-wise diffing adapts a dictionary to
the fine-tuning data or model \citep{bricken2024stagewise}. Our delta
dictionary is a single-layer, subtractive form of that idea: cheaper than a
crosscoder, blind to cross-layer structure, and a description of the
adapter's effect rather than of features shared with the base model. A recent
survey lists reconstruction error, dependence on the dictionary's training
data, and seed dependence among the open problems of sparse dictionary
methods \citep{sharkey2025open}; the checks of Sections~\ref{sec:robustness}
and Appendix~\ref{app:pkg4} test each of them for this dictionary.
Our contribution combines these tools with user-exposure controls and
input-location attribution for a benign-adapted anomaly score.
One-layer edits do not recover a complete circuit or fully test a
computational hypothesis as in causal scrubbing \citep{chan2022causal}.
The contribution is therefore the combined empirical audit and its
documented failure cases, not the individual tools or an unqualified
claim to be the first use of delta sparse coding.

\paragraph{Where the evidence lives.}
Code and result artifacts are in the
\href{https://github.com/SuramyaAngdembay/llm-sequence-mi-remote}{project repository}.
This edition uses revision \texttt{8a4e9d7};
\path{paper/companion/READABLE_COVERAGE.md}
provides the section mapping and per-result source paths. For the
factorial, the fold table and \texttt{anvil\_run\_summaries.txt} in
\path{results/train_matched_factorial/} are the reported evidence;
the nearby \texttt{roc/} files are earlier quick evaluations on a
different population. LANL metrics and measurement checks are in
\path{results/lanl_auth_replication/}. Raw datasets, pretrained weights,
and trained models are separate dependencies. The tracked summaries
support inspecting the contrasts; a fresh clone alone does not contain
every input needed to rerun all GPU experiments.
