...
# The cached r4.2 scores came from a batch-56 FORWARD pass; this run uses batch
# 1, which has NO padding at all. Measured on the 256-example pilot: the signed
# difference tracks sequence length (corr 0.39; shortest quartile -0.0093,
# longest +0.0046) - the padding signature, not a code difference. Model
# loading is identical (same BitsAndBytesConfig, dtype and PeftModel), and
# token counts match exactly. Batch 1 is therefore the cleaner estimate, and
# the cached value is not a gold standard to reproduce. Structural checks stay
# hard; agreement with the cache is REPORTED, and the anchor that matters is
# the metric-level comparison against the published ROC in step [4a].
...
echo "=== [2] GATE (structural hard; cache agreement reported) ==="
python - << PY
import json, sys
...
if r.get("nll_mean_abs_diff",9)>0.05: hard.append(f"mean|dNLL| {r.get('nll_mean_abs_diff'):.4f} too large to be batching")
...
print(f"GATE_PASSED (structural). vs batch-56 cache: mean|dNLL|={r['nll_mean_abs_diff']:.3e} "
      f"max={r['nll_max_abs_diff']:.3e} rankcorr={r['spearman_like_rank_corr']:.4f} - expected padding drift, see header")
...
echo "=== [4a] POOLED user-disjoint: the metric-level anchor ==="
python - << PY
...
rows=[]
for name,cls in list(CERT_VIEWS.items())+[("full_cached_batch56",None)]:
    if cls is None: s=d["cached"].to_numpy(float)
    else:
...
print("ANCHOR: compare the 'full' and 'full_cached_batch56' rows above against those.")
PY
