...
A train-matched retraining experiment then shows the profile is the
cause and not a correlate: at the headline eight-billion-parameter scale,
removing the profile block restores unseen-user ranking from chance to
about $0.90$ (paired resampling of the malicious users, $p = 0.005$),
while the same removal is inert at three billion parameters, so the harm
emerges with capacity and is carried by the personality line. On a
corporate login log with no profile of any kind, the collapse on unseen
users recurs and the shortcut relocates to the host field.
\end{abstract}
...
billion events), with $749$ red-team lateral-movement events over $104$
...
events, one text line per event, and called a window positive if it
contained a red-team event, so that truncation can never silently drop a
positive. We kept every red-team user and a one-in-ten sample of benign
...
identity anonymized, and user identity deranged across users. Positive
windows are rare enough that average precision is dominated by the base
rate, so we report AUC.

\begin{center}
\begin{tabular}{lcc}
\toprule
Identity fields in the serialization & Seen-user AUC & Unseen-user AUC \\
\midrule
...
Three observations. First, the seen-versus-unseen collapse reproduces on
real data with no injected profile: every serialization ranks seen users
near $0.95$ and unseen users near chance. Second, the user field is
inert: anonymizing or deranging it changes neither number, so the shortcut
is not carried by the user identifier. Third, the host field carries it,
with exactly the trade-off produced by CERT's profile lines: anonymizing
hosts costs seen-user ranking ($0.949\!\to\!0.877$) and buys unseen-user
ranking ($0.497\!\to\!0.627$). This is localization rather than failure.
People log on from their own machines, so on an authentication log the
host is the per-user-constant, identity-revealing field, and it plays the
role that CERT's personality line plays. A residual unseen-user gap
survives host removal ($0.63$ rather than $0.95$), which we read as
genuine behavioral heterogeneity across users rather than as shortcut.
Two caveats: each condition is a single 3B adapter, and the ground truth
is red-team credential theft rather than CERT's insider scenarios, so
these numbers replicate the mechanism rather than benchmark a detector.
One cross-dataset contrast is worth recording now and returns in
Section~\ref{sec:factorial}: the unseen-user collapse is already severe
at 3B here, whereas on CERT r6.2 it appears only at 8B; raw host identity
is a stronger shortcut than an injected profile at small scale, which is
what one expects if availability is a property of the data.
...
profile. The direct test is a controlled experiment at training time. We
...
are preserved but the link between profile and identity is destroyed. We
...
malicious users is held out in turn and ranked against $800$ benign test
users never used in training, a user's score is the maximum of its daily
scores, and the AUC is averaged over the four folds. Uncertainty is the
paper's primary instrument, the cluster bootstrap that resamples the
malicious users ten thousand times; because the same users are resampled
for every arm, it also yields a paired test of the difference between
two arms.
...
\emph{At the headline scale the collapse is causal and reversible.} The
8B \textsc{full} adapter collapses on unseen users (user-level AUC
$0.532$; day-level $0.399$), qualitatively reproducing the collapse of
Section~\ref{sec:collapse} on a capped-corpus adapter. Removing the
profile restores user-level AUC to $0.903$, a paired removal effect of
$+0.371$ whose bootstrap interval excludes zero ($p = 0.005$); the
bootstrap establishes the effect from a single adapter, so it does not
rest on seed replication. Removing the personality line alone recovers
just as much ($0.906$), so that line is the carrier and the
organizational header adds little. Profile availability during
adaptation is therefore causally responsible for the unseen-user
collapse, not a correlate of it.

\emph{At 3B the shortcut is captured but not yet harmful.} The 3B
\textsc{full} adapter already generalizes ($0.926$; its day-level AUC of
$0.741$ is in line with the better of the two repaired-serializer 3B
seeds of Section~\ref{sec:robustness}, $0.738$), and removal changes
nothing ($+0.03$, not significant). Yet the dictionary audit of the same
adapter is profile-dominated: four of its five selected features carry
close to $100$ percent profile mass. Capture without harm at 3B, capture
with harm at 8B: the shortcut's \emph{damage} to generalization is
capacity-emergent, the training-time face of the capacity explanation in
Section~\ref{sec:classical}. (The two repaired-serializer 3B r6.2
adapters differ widely in quality, $0.414$ versus $0.738$ day-level AUC,
so the 3B statement is a single-seed one.)

\emph{The 8B harm is concentrated, and psychometric familiarity predicts
it.} The 8B collapse is not uniform across the four malicious users: one
remains detected (user-level AUC $0.919$), two sit at chance ($0.527$),
and one is inverted, ranked $344$th of $407$ test users, among the least
anomalous. Removing the profile rescues every user at both scales
($0.82$ to $0.99$). A direct check of the mechanism against the $3{,}590$
benign training users: the inverted user has the least novel personality
profile of the four ($12$ benign users within $L_1 \le 10$ of it in the
five-dimensional Big Five space) and the user best detected at day level
has the most novel ($0$); the two most psychometrically familiar users
are exactly the two that collapse, while the organizational header is
not predictive (the surviving user shares its role, department, and team
with $29$ benign users). Ablation and novelty converge on the same
carrier: the 8B adapter memorizes psychometric familiarity strongly
enough that a malicious user whose scores resemble many benign trainees
is scored as familiar and ranked below the unfamiliar unseen benigns.
With four users this is consistent with, not proof of, the mechanism.

\emph{Shuffling.} Deranging profiles never helps. At 8B the
\textsc{shuffle} adapter sits with \textsc{full} at chance ($0.485$); at
3B it falls well below the honest-profile adapter ($0.656$ versus
$0.926$): a profile that lies misleads even a model for which an honest
profile is harmless.
...
tokens cheap to predict is preserved; only the correlation with true
identity is destroyed. Capture tracks predictability, which shuffling
leaves intact, while usefulness tracks whether the profile is informative
about identity, which shuffling removes: under \textsc{shuffle} the audit
...
by an out-of-setting prediction that the real-user dataset confirmed, by
a train-matched retraining experiment showing that profile availability
at adaptation time causes the unseen-user collapse at the headline scale
...
the second by the cross-detector comparison. The retraining experiment
adds a qualification the account did not anticipate: the shortcut's harm
is capacity-emergent, present in the smaller model's representation but
damaging only at the larger one.
...
the real-data studies are small; the retraining experiment uses one
adapter seed per scale, its significance coming from resampling the
malicious users rather than from seed replication; and the login-log
