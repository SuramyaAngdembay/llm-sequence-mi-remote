...
echo "Submitting Qwen3-8B r4.2 native active-control audit on A100..."
...
cat <<EOM
submitted_qwen3_8b_r42_native_active_control_gpu=1
causal_job=${CAUSAL_JOB}
bootstrap_job=${BOOTSTRAP_JOB}
partition=${GPU_PARTITION}
account=${GPU_ACCOUNT}
...
After completion, check resource usage with:
  sacct -j ${CAUSAL_JOB},${BOOTSTRAP_JOB} --format=JobID,JobName,Partition,State,Elapsed,ReqMem,MaxRSS,AveRSS,ExitCode -P
EOM
