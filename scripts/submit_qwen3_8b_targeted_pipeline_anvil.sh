...
TRAIN_JOB=$(
  sbatch --parsable \
    "${TRAIN_DEPENDENCY_ARGS[@]}" \
    --export=ALL,REPO_DIR="$REPO_DIR",CONDA_ENV="$CONDA_ENV",CONFIG="$CONFIG",DATA_DIR="$DATA_DIR",OUTPUT_DIR="$CHECKPOINT_ROOT",NPROC="$NPROC",MICRO_BS="$MICRO_BS",GRAD_ACCUM="$GRAD_ACCUM",DL_WORKERS="$DL_WORKERS",ATTN_IMPL="$ATTN_IMPL",GC_MODE="$GC_MODE",MAX_TRAIN="$MAX_TRAIN",MAX_VAL="$MAX_VAL",SAVE_STEPS="$SAVE_STEPS",EVAL_STRATEGY="$EVAL_STRATEGY",EVAL_STEPS="$EVAL_STEPS",RESUME_FROM_CHECKPOINT="$RESUME_FROM_CHECKPOINT",IGNORE_DATA_SKIP="$IGNORE_DATA_SKIP" \
    slurm/train_qlora_ddp.template.sbatch
)

EXTRACT_JOB=$(
  sbatch --parsable --dependency=afterok:${TRAIN_JOB} \
    --export=ALL,REPO_DIR="$REPO_DIR",CONDA_ENV="$CONDA_ENV",CONFIG="$CONFIG",DATA_DIR="$DATA_DIR",ADAPTER_DIR="${CHECKPOINT_ROOT}/adapter",OUTPUT_DIR="$TOKEN_CACHE_ROOT",LAYERS="$EXTRACT_LAYERS",BATCH_SIZE="$EXTRACT_BATCH_SIZE",CHUNK_EXAMPLES="$EXTRACT_CHUNK_EXAMPLES",MAX_EXAMPLES="$EXTRACT_MAX_EXAMPLES" \
    slurm/extract_token_deltas.template.sbatch
)

SAE_JOB=$(
  sbatch --parsable --dependency=afterok:${EXTRACT_JOB} \
    --export=ALL,REPO_DIR="$REPO_DIR",CONDA_ENV="$CONDA_ENV",EXTRACT_DIR="$TOKEN_CACHE_ROOT",OUTPUT_DIR="$FRONTIER_ROOT",LAYERS="$EXTRACT_LAYERS",MAX_ROWS="$SAE_MAX_ROWS",BATCH_SIZE="$SAE_BATCH_SIZE",LATENT_MULTIPLIERS="$LATENT_MULTIPLIERS",TOPK_VALUES="$TOPK_VALUES" \
