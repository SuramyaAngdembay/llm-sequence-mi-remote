...
TRAIN_JOB=$(
  sbatch --parsable \
    --export=ALL,REPO_DIR="$REPO_DIR",CONDA_ENV="$CONDA_ENV",CONFIG="$CONFIG",DATA_DIR="$DATA_DIR",OUTPUT_DIR="$CHECKPOINT_ROOT",NPROC="$NPROC",MICRO_BS="$MICRO_BS",GRAD_ACCUM="$GRAD_ACCUM",DL_WORKERS="$DL_WORKERS",ATTN_IMPL="$ATTN_IMPL",GC_MODE="$GC_MODE",MAX_TRAIN="$MAX_TRAIN",MAX_VAL="$MAX_VAL" \
