...
ADAPTER_DIR="${ADAPTER_DIR:-/anvil/projects/x-cis230270/x-sangdembay/cert-qlora-MI/checkpoints/qwen3_8b_session_qlora_ddp/adapter}"
EXTRACT_DIR="${EXTRACT_DIR:-/anvil/projects/x-cis230270/x-sangdembay/cert-qlora-MI/token_delta_cache/qwen3_8b_session_token_deltas_targeted}"
FRONTIER_DIR="${FRONTIER_DIR:-/anvil/projects/x-cis230270/x-sangdembay/cert-qlora-MI/outputs/token_delta_sae_frontier_qwen3_8b}"

LAYER="${LAYER:-18}"
LATENT_MULT="${LATENT_MULT:-2}"
TOPK="${TOPK:-8}"
TAG="${TAG:-l${LAYER}_m$(printf '%02d' "$LATENT_MULT")_k$(printf '%02d' "$TOPK")}"
OUTPUT_DIR="${OUTPUT_DIR:-/anvil/projects/x-cis230270/x-sangdembay/cert-qlora-MI/outputs/token_delta_sae_causal_qwen3_8b/${TAG}}"
...
CAUSAL_JOB=$(
  sbatch --parsable \
    --export=ALL,REPO_DIR="$REPO_DIR",CONDA_ENV="$CONDA_ENV",CONFIG="$CONFIG",DATA_DIR="$DATA_DIR",ADAPTER_DIR="$ADAPTER_DIR",EXTRACT_DIR="$EXTRACT_DIR",FRONTIER_DIR="$FRONTIER_DIR",OUTPUT_DIR="$OUTPUT_DIR",LAYER="$LAYER",LATENT_MULT="$LATENT_MULT",TOPK="$TOPK",BATCH_SIZE="$BATCH_SIZE",SAE_BATCH_SIZE="$SAE_BATCH_SIZE",CONTEXT_MODES="$CONTEXT_MODES",TOP_SETS="$TOP_SETS",CONTROL_SET="$CONTROL_SET",ALPHAS="$ALPHAS",MAX_RECEIVERS="$MAX_RECEIVERS",MAX_CANDIDATE_DONORS="$MAX_CANDIDATE_DONORS" \
    slurm/eval_token_delta_sae_causal.template.sbatch
)

BOOTSTRAP_JOB=$(
  sbatch --parsable --dependency=afterok:${CAUSAL_JOB} \
    --export=ALL,REPO_DIR="$REPO_DIR",CONDA_ENV="$CONDA_ENV",BEST_ROWS_CSV="${OUTPUT_DIR}/token_delta_sae_causal_best_rows.csv",OUT_DIR="$BOOTSTRAP_DIR",TOP_SETS="$TOP_SETS",CONTROL_SET="$CONTROL_SET",N_BOOTSTRAP="$N_BOOTSTRAP",SEED="$SEED" \
    slurm/bootstrap_token_delta_sae_causal_cpu.template.sbatch
)

cat <<EOM
submitted_qwen3_8b_token_eval_bundle=1
