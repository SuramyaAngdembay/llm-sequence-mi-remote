#!/bin/bash
# Per-fold causal+necessity evals for the SES-restricted (behavioral) r6.2
# feature sets, LOUO discipline. Batch settings from probe 19406535.
set -euo pipefail
...
for USER_ID in ACM2278 CDE1846 CMP2946 MBG3183; do
  export FRONTIER_DIR=$P/outputs/token_delta_sae_frontier_r62_behavioral_louo/louo_${USER_ID}
  export RECEIVER_USER_FILE=$P/outputs/user_splits_r62/louo_${USER_ID}_confirmation_users.txt

  export OUTPUT_DIR=$P/outputs/token_delta_sae_causal_qwen3_8b_r62_behavioral_louo/louo_${USER_ID}
  export EXCLUDE_SAME_USER_DONORS=1
  mkdir -p "$OUTPUT_DIR"
  if [ "$USER_ID" = "CDE1846" ]; then T=01:30:00; else T=01:00:00; fi
  sbatch -p gpu -A cis230270-gpu -t $T -J bcausal_${USER_ID} --export=ALL \
    slurm/eval_token_delta_sae_causal.template.sbatch

  export OUTPUT_DIR=$P/outputs/token_delta_sae_necessity_qwen3_8b_r62_behavioral_louo/louo_${USER_ID}
  export EXCLUDE_SAME_USER_MATCHES=1 MAX_PAIRS=0
  mkdir -p "$OUTPUT_DIR"
  sbatch -p gpu -A cis230270-gpu -t 00:30:00 -J bnecess_${USER_ID} --export=ALL \
