...
# Short A100 gpu-debug probe for the r6.2 Qwen3-8B token-necessity run.
# This validates the real necessity evaluator path, including SAE decode and
# model patch scoring, without spending H100 SUs or launching the full job.
...
COMMON_OUTPUT_ROOT="${COMMON_OUTPUT_ROOT:-/anvil/projects/x-cis230270/x-sangdembay/cert-qlora-MI/outputs/token_delta_sae_necessity_probe_qwen3_8b_gpu_debug}"
LAYER="${LAYER:-18}"
...
PATCH_CHUNK_SIZE="${PATCH_CHUNK_SIZE:-8}"
...
export OUTPUT_DIR LAYER LATENT_MULT TOPK BATCH_SIZE SAE_BATCH_SIZE PATCH_CHUNK_SIZE TOKEN_DELTA_DTYPE
export CONTEXT_MODES TOP_SETS CONTROL_SET ACTIVE_CONTROL_MIN_FRAC ALPHAS MAX_PAIRS GPU_POLL_SEC

echo "Submitting r6.2 Qwen3-8B token-necessity gpu-debug probe..."
...
cat <<EOM
submitted_qwen3_8b_token_necessity_gpu_debug_probe=1
probe_job=${PROBE_JOB}
partition=${GPU_DEBUG_PARTITION}
account=${GPU_ACCOUNT}
output_dir=${OUTPUT_DIR}
